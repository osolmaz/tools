...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Scatter one image into a balanced, non-overlapping tile pattern."
    )
    parser.add_argument("source", type=Path, help="PNG, JPEG, WebP, or SVG tile image")
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    output = Path(args.output) if args.output else source.with_name(f"{source.stem}-tiled.png")
    canvas_size = CanvasSize(width=args.width, height=args.height)
    result = render_tiles(load_tile(source), PRESETS[args.preset], canvas_size)
