...
```bash
gh search prs --owner ORG --author USER --updated ">=YYYY-MM-DDTHH:MM:SSZ" --json repository,number,title,state,isDraft,author,createdAt,updatedAt,closedAt,mergedAt,url
gh search prs --owner ORG --involves USER --updated ">=YYYY-MM-DDTHH:MM:SSZ" --json repository,number,title,state,isDraft,author,createdAt,updatedAt,closedAt,mergedAt,url
