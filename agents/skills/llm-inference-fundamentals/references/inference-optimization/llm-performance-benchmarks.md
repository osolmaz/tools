...
- **Measuring optimization gains**. Inference techniques such as
  ...
  [disaggregated serving](prefill-decode-disaggregation.md),
  or [KV cache offloading](kv-cache-offloading.md) should
...
- **Latency**: How quickly the model responds to a request. Key latency metrics
  ...
  See [LLM inference metrics](../llm-inference-basics/llm-inference-metrics.md) for
  more information.
