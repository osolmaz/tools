...
- **Measuring optimization gains**. Inference techniques such as [speculative decoding](speculative-decoding.md), [prefix caching](prefix-caching.md), [disaggregated serving](prefill-decode-disaggregation.md), or [KV cache offloading](kv-cache-offloading.md) should always be validated with repeatable performance testing.
...
- **Latency**: How quickly the model responds to a request. Key latency metrics include TTFT, ITL, median latency, and tail latencies (P95, P99). These determine how responsive your LLM feels to users.
  ...
  See [LLM inference metrics](../llm-inference-basics/llm-inference-metrics.md) for more information.
