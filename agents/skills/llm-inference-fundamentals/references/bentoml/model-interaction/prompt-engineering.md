...
LLM inference relies heavily on the [KV cache (Key-Value cache)](../llm-inference-basics/how-does-llm-inference-work.md#prefill) to speed up generation.
...
Prompt engineering is often tuned together with [inference parameters](inference-parameters.md). The prompt defines the task and constraints, while parameters such as `temperature`, `top_p`, and `max_tokens` control how the model samples and bounds the response.
...
- [Prefix caching](../inference-optimization/prefix-caching.md) to reuse shared prompt segments
- [KV cache offloading](../inference-optimization/kv-cache-offloading.md) to move cache data to low-cost storage when needed
