...
Inference parameters are the settings you pass with an LLM request to control how the model generates responses. They do not change the model weights. Instead, they impact the [decoding process](../llm-inference-basics/how-does-llm-inference-work.md), such as:
...
Some parameters mainly affect output quality and style, while others have direct serving implications for scheduling, [throughput](../llm-inference-basics/llm-inference-metrics.md), memory usage, and production cost.

## Common inference parameters

You will see these parameters in hosted APIs, [OpenAI-compatible servers](openai-compatible-api.md), [inference frameworks](../getting-started/choosing-the-right-inference-framework.md) like vLLM and SGLang, and agentic frameworks. Here is a quick summary of the common ones:
...
- Use low temperature for factual question answering, extraction, classification, and [structured workflows](structured-outputs.md).
...
Low temperature does not guarantee factual accuracy. It only reduces randomness in the decode step. A model can still give a confident wrong answer if the [prompt](prompt-engineering.md) lacks grounding, the model lacks knowledge, or the application does not verify outputs.
...
> The [top-p and top-k visualizer](https://bentoml.com/llm/model-interaction/inference-parameters/) is available on the rendered handbook.
...
- Use [greedy decoding](../llm-inference-basics/how-does-llm-inference-work.md#how-are-tokens-selected-via-sampling) if you want the model to select the highest probability token at each step. It is deterministic in a fixed serving setup, but can be prone to repetition.
...
Length parameters directly affect [latency](../llm-inference-basics/llm-inference-metrics.md) and cost because LLMs generate one token at a time during decode.
...
For inference systems, output length also affects scheduling. Long generations hold active request state longer, consume [KV cache](../llm-inference-basics/how-does-llm-inference-work.md#prefill) longer, and can interfere with latency-sensitive traffic.
...
- Stop after one list item, one SQL statement, or one [tool call](function-calling.md).

Stop sequences are not a replacement for schema enforcement. They only end generation when a sequence appears. If you need guaranteed JSON, use [structured outputs](structured-outputs.md) or constrained decoding when your provider supports it.
...
Use advanced controls when the output is consumed by software. For example, extraction pipelines, [function calling](function-calling.md), and [structured data generation](structured-outputs.md) usually need stronger guarantees than prompt instructions alone can provide.
...
For production systems, parameter tuning is also an infrastructure concern. For example, higher output lengths and more exploratory sampling can increase [latency](../llm-inference-basics/llm-inference-metrics.md), GPU utilization, KV cache pressure, and tail latency under load.
