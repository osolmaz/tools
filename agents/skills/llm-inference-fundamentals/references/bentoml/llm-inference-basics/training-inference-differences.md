...
> The [LLM lifecycle visualizer](https://bentoml.com/llm/llm-inference-basics/training-inference-differences/) is available on the rendered handbook.
...
LLM inference means applying the trained model to new data to make predictions. Unlike training, inference [happens continuously and in real-time](what-is-llm-inference.md), responding immediately to user input or incoming data. It is the phase where the model is actively "in use." Better-trained and more finely-tuned models typically provide more accurate and useful inference.
...
Learn more about the difference between [serving and inference](what-is-llm-inference.md#whats-the-difference-between-serving-and-inference).
...
Fine-tuning is a form of training. You update some of the model’s weights using new data to adapt it to a specific task or domain. Inference doesn’t change any weights. It only uses the model to generate predictions. See the [fine-tuning section](../model-preparation/llm-fine-tuning.md) to learn more.
