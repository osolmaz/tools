...
LLM inference refers to using trained LLMs, such as GPT-5, GLM-5, and DeepSeek-V3.2, to generate meaningful outputs from user inputs, typically provided as natural language [prompts](../model-interaction/prompt-engineering.md). During inference, the model processes the prompt through its vast set of parameters to generate responses like text, code snippets, summaries, and translations.
...
Use the visualizer below to see how a request flows during LLM inference. For details, learn [how LLM inference works](how-does-llm-inference-work.md).

> The [request lifecycle visualizer](https://bentoml.com/llm/llm-inference-basics/what-is-llm-inference/) is available on the rendered handbook.
...
Popular [inference frameworks](../getting-started/choosing-the-right-inference-framework.md) include vLLM, SGLang, MAX, and TensorRT-LLM. They’re designed to maximize GPU efficiency while making LLMs easier to deploy at scale.
...
- [Continuous batching](../inference-optimization/static-dynamic-continuous-batching.md): Dynamically grouping requests for better GPU utilization
- [KV cache management](../inference-optimization/kv-cache-offloading.md): Reusing or offloading attention caches to handle long prompts efficiently
- [Speculative decoding](../inference-optimization/speculative-decoding.md): Using a smaller draft model to speed up token generation
- [Quantization](../model-preparation/llm-quantization.md): Running models in lower precision (e.g., INT8, FP8) to save memory and compute
- [Prefix caching](../inference-optimization/prefix-caching.md): Caching common prompt segments to reduce redundant computation
- [Multi-GPU distribution/Parallelism](../inference-optimization/data-tensor-pipeline-expert-hybrid-parallelism.md): Splitting LLMs across multiple GPUs for larger context windows
...
Learn more in the [inference optimization](../inference-optimization/index.md) chapter.
...
Understanding LLM inference (e.g., [batching](../inference-optimization/static-dynamic-continuous-batching.md), [caching](../inference-optimization/prefix-caching.md), [quantization](../model-preparation/llm-quantization.md) and [routing](../inference-optimization/inference-routing.md)) early gives you a clear edge. It helps you evaluate the features of different model service providers and [inference frameworks](../getting-started/choosing-the-right-inference-framework.md), so you can make smarter choices, avoid surprises, and build more scalable systems.
...
For more information, see [serverless vs. self-hosted LLM inference](../getting-started/serverless-vs-self-hosted-llm-inference.md).
