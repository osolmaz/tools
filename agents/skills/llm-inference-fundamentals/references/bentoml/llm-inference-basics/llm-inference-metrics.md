...
> The [latency metrics playground](https://bentoml.com/llm/llm-inference-basics/llm-inference-metrics/) is available on the rendered handbook.
...
- **Tokens per Second (TPS)**: This metric provides a finer-grained view of throughput by measuring how many tokens are processed every second across all active requests. It comes in two forms:
    ...
    - A chatbot that generates long replies from short [prompts](../model-interaction/prompt-engineering.md) (e.g., 20-token prompt → 500-token response) depends heavily on output TPS.
