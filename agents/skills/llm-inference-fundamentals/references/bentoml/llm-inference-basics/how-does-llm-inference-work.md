...
A key metric to monitor for prefill is the Time to First Token (TTFT), which measures the latency from prompt submission to first token generation. More details will be covered in the [inference optimization](../inference-optimization/index.md) chapter.
...
> The [token-by-token decode visualizer](https://bentoml.com/llm/llm-inference-basics/how-does-llm-inference-work/) is available on the rendered handbook.
...
> The [latency timeline visualizer](https://bentoml.com/llm/llm-inference-basics/how-does-llm-inference-work/) is available on the rendered handbook.
...
The open-source community is actively working on different strategies to separate prefill and decode. For more information, see [prefill-decode disaggregation](../inference-optimization/prefill-decode-disaggregation.md).
...
> The [context-window simulator](https://bentoml.com/llm/llm-inference-basics/how-does-llm-inference-work/) is available on the rendered handbook.
...
As mentioned above, LLMs use the KV cache from previous tokens to avoid fully reprocessing everything in the decode phase, which helps with latency. When reusing the KV cache across multiple requests, it is more accurate to call this technique [prefix caching](../inference-optimization/prefix-caching.md).
...
Learn more about [LLM inference parameters](../model-interaction/inference-parameters.md).
...
This is why [inference optimization](../inference-optimization/index.md) is a major focus in production systems.
