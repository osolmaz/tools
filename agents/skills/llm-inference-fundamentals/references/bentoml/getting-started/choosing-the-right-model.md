...
> The [model explorer](https://bentoml.com/llm/getting-started/choosing-the-right-model/) is available on the rendered handbook.
...
This process teaches the models how to follow user [prompts](../model-interaction/prompt-engineering.md) more reliably, so that they are better aligned with human expectations. They understand task intent and respond more coherently to commands like:
...
Learn more about [multi-model inference pipelines](../infrastructure-and-operations/multi-model-inference-pipelines.md).
...
- Pass that token to your [inference framework](choosing-the-right-inference-framework.md) or deployment environment (e.g., via an environment variable like `HF_TOKEN`)
...
- Some models have [“quantized” versions](../model-preparation/llm-quantization.md), meaning the model weights are compressed to reduce memory usage.
