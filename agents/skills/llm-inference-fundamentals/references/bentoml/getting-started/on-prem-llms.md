...
- **Predictable costs**: After the initial hardware investment, ongoing inference costs may be lower than usage-based serverless APIs. However, you may need to apply certain inference optimization techniques like [KV cache offloading](../inference-optimization/kv-cache-offloading.md) and [prefix caching](../inference-optimization/prefix-caching.md) to stay within budget.
...
| **Maintenance and operations**  | Requires dedicated teams for infra, LLM-specific observability, and updates | Faster setup, but still require some infra management; [BYOC](bring-your-own-cloud.md) offloads part of the burdens to service providers |
