...
The [KV cache](../inference-optimization/kv-cache-offloading.md) is usually the largest runtime overhead. It stores attention keys and values so the model can reuse previous tokens during decoding instead of recomputing them. The cache grows with sequence length and the number of active requests. Serving engines also need memory for temporary activations, workspace buffers, framework allocations, and sometimes CUDA graphs. If you leave only a tiny amount of GPU memory beyond the weights, you will be limited to short contexts, small batches, and low concurrency.
...
> The [GPU memory calculator](https://bentoml.com/llm/getting-started/calculating-gpu-memory-for-llms/) is available on the rendered handbook.
