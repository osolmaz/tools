...
Unlike [quantization](llm-quantization.md), which reduces the precision of existing weights, distillation produces an entirely new model. The two techniques are complementary. A distilled model can also be quantized for further efficiency.
...
- [Inference optimizations](../inference-optimization/index.md) (like [prefix caching](../inference-optimization/prefix-caching.md)) make serving more efficient
