...
- **Less data movement**. LLM decoding is often limited by GPU memory bandwidth because the runtime repeatedly reads model weights while generating tokens. Lower-precision weights mean fewer bytes to be moved from [GPU memory to the compute units](../kernel-optimization/gpu-architecture-fundamentals.md), which can reduce per-token latency.
...
A smaller weight footprint also leaves more GPU memory available for the [KV cache](../llm-inference-basics/how-does-llm-inference-work.md#the-two-phases-of-llm-inference), larger batches, and more concurrent requests. Weight quantization does not reduce the KV cache size per token by itself. It requires quantizing the KV cache separately.
...
> The [quantization visualizer](https://bentoml.com/llm/model-preparation/llm-quantization/) is available on the rendered handbook.

This calculator estimates **weight memory only**. Use the [GPU memory calculator](../getting-started/calculating-gpu-memory-for-llms.md) to estimate your overall requirements.
