...
1. Implement the given plan end-to-end. If there is no plan markdown document for the task which the skill is triggered for, create a `.md` file in `~/scratch` for that plan and then proceed with implementation. If context compaction happens, make sure to re-read the plan to stay on track. Finish to completion. If there is a PR open for the implementation plan, do it in the same PR. If there is no PR already, open PR.
