...
1. Implement the given plan end-to-end. If there is no plan markdown document for the task which the skill is triggered for, create a `.md` file for that plan in the scratch repo at `~/repos/scratch` and then proceed with implementation. Keep the plan outside the implementation repo unless the user explicitly asks to track it there. If context compaction happens, make sure to re-read the plan to stay on track. Finish to completion. If there is a PR open for the implementation plan, do it in the same PR. If there is no PR already, open PR. Before finishing, commit and push any new or changed plan file in the `~/repos/scratch` repo unless the user asked not to.
