...
async function reproduceBugAndTestFix(pr, validationPath) {
  ...
  if (codeFiles.length === 0) {
    return {
      validation_status: "fix_not_proven",
      route: "comment_and_escalate_to_human",
      summary: "Could not isolate a non-test code change to ablate while keeping the new validation intact.",
      repro_steps: [],
  ...
  const mergeBase = (await runCommand("git", ["-C", pr.workdir, "merge-base", "HEAD", baseRef]))
    .stdout.trim();
  const patch = (
    await runCommand("git", [
      "-C",
  ...
  if (!initial.ok) {
    return {
      validation_status: "fix_not_proven",
      route: "comment_and_escalate_to_human",
      summary: "The targeted validation did not pass on the PR head before ablation, so the fix could not be proven.",
      repro_steps: [],
      targeted_tests: testPlan.commands,
      integration_tests: [],
      e2e_tests: [],
...
async function collectReviewState(pr) {
  ...
  const reviewComments = await ghApiJson(`repos/${pr.repo}/pulls/${pr.prNumber}/comments?per_page=100`);
  const issueComments = await ghApiJson(`repos/${pr.repo}/issues/${pr.prNumber}/comments?per_page=100`);
...
async function collectCiState(pr) {
  const prView = await ghPrView(pr.repo, pr.prNumber, ["statusCheckRollup", "commits", "isCrossRepository"]);
  const headSha =
    String(prView?.commits?.[0]?.oid ?? pr.headSha) || pr.headSha;
...
async function postClosePr(pr, commentStep) {
  ...
  await runCommand("gh", ["pr", "comment", String(pr.prNumber), "--repo", pr.repo, "--body-file", commentFile]);
  await runCommand("gh", ["pr", "close", String(pr.prNumber), "--repo", pr.repo, "--comment", "Closed by automated triage."]);
  return {
    route: "close_pr",
...
function promptDoSuperficialRefactor(pr, outputs) {
  return [
    "You are still in the same PR session inside the isolated workspace.",
    `Target PR: ${prRef(pr)}`,
...
function promptReviewLoop(pr, outputs) {
  const reviewStatePath = outputs.collect_review_state?.review_state_path ?? `${FLOW_DIR}/review-state.json`;
  const validation = outputs.reproduce_bug_and_test_fix ?? outputs.test_feature_directly ?? null;
  return [
