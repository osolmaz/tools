...
function promptReviewLoop(pr, hadSuperficialRefactor) {
  return [
    ...
    "Then determine the correct base branch or merge base from the checked-out repo and run a fresh local `codex review --base <base>` against the PR work. Review only the PR delta, not the whole repository state and not a stale local diff.",
