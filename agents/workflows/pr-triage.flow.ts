...
function promptReviewLoop(pr, hadSuperficialRefactor) {
  return [
    ...
    "Then run a fresh local `codex review` yourself against the PR work isolated from the correct base commit or merge base. Review only the PR delta, not the whole repository state and not a stale local diff.",
    "Use the checked-out repo to determine the correct base branch or merge base before running local review.",
